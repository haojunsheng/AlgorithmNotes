 ...
 * 步骤：
 * f(i)表示以第i个字符为结尾不包含重复字符的子字符串的最长长度
 *
 * 1.求包含第i个字符的最长无重复子串，可以转化为求包含i-1个字符的无重复子串。
 * 2.状态转移方程：dP[i],i记录遍历到第几个字符串来了，起始值为1.
 * dP[i]的值记录了第i个无重复子串的起始位置。则在第i个字符的无重复子串的值就为i-DP[i]。这个DP方程可能有点奇怪，但是空间复杂度为O(n),只需要遍历一次。效率比较高。
 * 状态转移方程有两种情况：
