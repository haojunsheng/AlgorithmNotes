 ...
 * 比如图a所表示的链表中，我们要删除节点i，可以从链表的头节点a开始顺序遍历，发现节点h的m_PNext指向要删除的节点i，于是我们可疑把节点h的m_PNext指向i的下一个节点即为j。
 * 指针调整之后，我们就可以安全地删除节点i并保证链表没有断开。这种思路由于需要顺序查找，时间复杂度自然就是O（n)了。
 * <p>
 * 之所以要从头开始查找，是因为我们需要得到被删除的节点的前面一个节点。在单链表中，节点中没有前一个节点的指针，所以只好从链表的头结点开始顺序查找。
 * 那是不是一定要得到被删除的节点的前一个节点呢？答案是否定的。我们可以很方面地得到要删除的节点的下一个节点。
