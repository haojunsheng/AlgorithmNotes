 ...
 * 2、二分查找缩小查找范围依赖于中间值和待查找值的比较。
 * 设置两个指针，index1指向第一个元素，index2指向最后一个元素。
 * 如果中间indexMid指向的元素大于或者等于index1指向的元素，那么，最小的元素一定在indexMid之后。
 * 如果indexMid指向的元素小于或者等于index2指向的元素，那么最小元素一定在indexMid之前或者就是indexMid所指向的元素。
 * 就是利用这种判断条件来不断缩小查找范围。
 * 3、结束条件：
